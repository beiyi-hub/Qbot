...
with open("./set.json", "r", encoding="utf-8") as setting:  # 读取长期保存的设置
    ...
    voice_name = setdir["voice_model"]
    SoVITS_version= setdir["SoVITS_version"]
    Reference_a_path = setdir["audio_path"]
...
5. 当你需要通过B站搜索视频时，你需要使用以下格式去搜索， #cut##bilibili/搜索关键词/##cut#，搜索词应当尽量精简，调用搜索功能时，不要有任何其他回复，仅使用固定格式回答,搜索词不应该断句
6. 群聊中@群友时，需要按照格式#cut#[CQ:at,qq=user_id,name=对方名称]后面紧跟你想表达的内容#cut#；#cut#[CQ:at,qq=1901182260,name=春日]亲爱的，在不在？#cut#  (不知道id时，请勿引用)
7. 当你想使用语音进行回复时，请使用以下格式#cut##voice/语音内容##cut#，例如：“#cut##voice/你好，亲爱的#cut#” （语音内容中不要携带任何特殊字符，以及emoji）
...
10. 请仔细辨别用户发送对象，如“【行己之道】说：幻蓝，在干什么呢”，这句话不是对你的提问，而是对幻蓝的提问，你不需要参与回复，空回复即可。
13. 请仔细辨别消息的发送者，例如：“【行己之道】说：[CQ:at,qq=2094927307,name=柴郡猫猫] 春日：抱抱你猫猫”，在这句话中只有最前面的名字“行己之道”才是消息发送者，后面再出现的名字一律忽视，仅以消息来源者为准。
14. 当你需要去网上找图时，可以使用以下格式回复 #cut##image/图片关键词##cut# ，例如查找柴郡的图： #cut##image/柴郡##cut#，不要和绘画功能弄混,不要有任何多余解释，只填充关键字，使用中文。
15. 在群聊或私聊中遇到重要的信息（包括但不限于：自认为重要的事情，对各个用户的印象，和群友讨论过，发生过的趣事等等）按照以下格式进行记忆，例如："#cut#原来小春日是这样的人哦#cut##summary/春日喜欢涩涩##cut#" （与该用户对话文本较少时，请勿总结）
"""
...
def request_api(data):
    ...
    try:
            ...
            for line in lines:
                if line.startswith("data: "):
                    ...
                    try:
                        ...
                        print(f"JSON 解析错误: {e}")
            condition = False
            if len(assistant_reply) > 500:
                words = [word for word in jieba.lcut(assistant_reply) if len(word) > 1]
                word_count = {}
                for word in words:
        ...
        if "think" in assistant_reply:
            ...
            print(123)
            if "<thinking>" in assistant_reply:
                pattern = r'<thinking>.*?</thinking>\s*'
            else:
                pattern = r'```thinking.*?```\s*'
            return re.sub(pattern, '', assistant_reply, flags=re.DOTALL),assistant_reply
        else:
...
def get_pixiv(keyword):
    ...
    crawler = get_Pixiv.PixivCrawler(username, password, use_selenium=True)
    page_limit = 1
    ...
    try:
        if draw==True:
            ...
            if response.status_code == 200:
                ...
                send_msg({'msg_type': 'group', 'number': to, 'msg': '绘画完毕发送中...'})
                print("成功")
                send_msg({'msg_type': 'group', 'number': to, 'msg':fr"[CQ:image,file={absolute_path}\data\image\{n}.png]"})
    except Exception as e:
            print('绘画错误:', e)
            send_msg({'msg_type': 'group', 'number': to, 'msg': 'AI绘画操作无法执行'})
...
def voice(tts_content):
    if SoVITS_version == "1":
        voice1 = tts_content.replace("```", "").replace("#", "").replace(
            "cut", "").replace("[CQ:face,id=", "").replace("]", "").replace("1", "").replace("2", "").replace("3",
                                                                                                              "").replace(
            "4", "").replace("5", "").replace("6", "").replace("7", "").replace("8", "").replace("9", "").replace("0", "")
        print(voice1)
        print(9999)
        tts_data = {
            "cha_name": voice_name,
        ...
        return name
    if SoVITS_version == "2" or SoVITS_version == "3" or SoVITS_version == "4":
...
def main(rev):
    ...
    global system
    global t
    global temp_think_data
    global temp_msg_container
    try:
        timestamp = time.time()
        ...
        if rev['sub_type'] == 'poke' and rev['target_id'] == rev['self_id']:
            ...
            if poke == True:
                ...
                #           'msg': f"[CQ:image,file={absolute_path}\哦润吉.jpg]"})
                print("收到戳一戳")
                data = {
                ...
                content,temp_think_data = request_api(data)
                print(content)
            ...
            if "[CQ:image," in rev['raw_message']:
                ...
                with open(image, 'rb') as img_file:
                    p_img_base = base64.b64encode(img_file.read()).decode('utf-8')
                print("运行到这里说明保存成功")
            ...
            if True:
                ...
                if '#reset' in rev['raw_message']:
                    ...
                    if pause_private != True:
                        ...
                        print(s_memory)
                        try:
                            with open ("./user/p%s/i_memory.txt" % rev["sender"]["user_id"] , "r", encoding="utf-8") as f:
                                i_memory = f.read()
                            print(i_memory)
                        except:
                            i_memory = ""
                            pass
                        print("重要记忆：" + i_memory)
                        if "[CQ:image" not in rev['raw_message'] and p_img_base == "":
                            data = {
                                "model": user_chat_model,
                                "messages": merge_contents([{"role": "system", "content": messages[0][
                                                                                              "content"] + "[memory](模糊 无时效性)\n%s\n" % s_memory + i_memory + "你现在是在和用户私聊" + e_information}] + messages[
                                                                                                                                                                          1:]),
                                "stream": True,
                            }
                        elif p_img_base != "":
                            ...
                            data = {
                                ...
                                "messages": [{"role": "system",
                                              "content": "你是一个图像分析专家，你能结合你的所知准确完整的分析图片内容"},
                                             {"role": "user",
                                              "content": [
                                                  {"type": "text",
                                                   "text": "请尝试详细分析这张图片里的元素和内容"},
                                                  {"type": "image_url",
                                                   "image_url": {
                                                       "url": f"data:image/png;base64,{p_img_base}"
                                                   }}]}],
                                "stream": True,
                            }
                            recognize_msg , temp_think_data = request_api(data)
                            data = {
                                "model": user_chat_model,
                                "messages": merge_contents([{"role": "system", "content": messages[0][
                                                                                              "content"] + "[memory](模糊 无时效性)\n%s\n" % s_memory + i_memory + "你现在是在和用户私聊" + e_information}] + ["%s给你发了一张图片，以下是关于这张图片的描述，请你对此做出回应。\n%s" % (
                                              rev["sender"]["nickname"], recognize_msg)]),
                                "stream": True,
                            }
                            p_img_base = ""
                        is_return = True
                        while is_return:
                            ...
                            is_return = False
                            try:
                                processed_d_data1,temp_think_data= request_api(data)
                            except:
                                pass
                            ...
                            if len(temp_tts_list)==1:
                                ...
                                if '#voice/' in temp_tts_list[-2]:
                                    name = voice(temp_tts_list[-2].replace("#", "").replace("#voice/", "").replace("voice", ""))
                                    send_voice(
                                        {'msg_type': 'group', 'number': rev['group_id'], 'msg': name})
                                    temp_tts_list[-2] = ""
                                if '#summary/' in temp_tts_list[-2]:
                                    with open(
                                            "./user/p%s/i_memory.txt" % rev["sender"]["user_id"],
                                            "a",
                                            encoding="utf-8")as f:
                                            f.write(temp_tts_list[-2].replace("#", "").replace("#summary/",""))
                                    temp_tts_list[-2] = ""
                                ...
                                if len(rev['raw_message']) < 100:
                                    objdict["illue%s" % rev["sender"]["user_id"]][0] = \
                                    objdict["illue%s" % rev["sender"]["user_id"]][0] + [
                                        {'role': 'user', 'content': rev['raw_message']},
                                        {'role': 'assistant', 'content': temp_think_data}]
                                    with open(
            ...
            img_base = {}
            img_base[rev['group_id']] = ""
            temp_msg_container[rev['group_id']] = ("【"+"name="+
                        rev["sender"]["nickname"] + "qq="+str(rev["sender"]["user_id"]) +"】说：" + rev['raw_message'].replace(
                    '[CQ:at,qq=%d]' % rev['self_id'], ''))      #创建一个临时的消息容器，如果没有触发回复，就将消息放入容器中
            if rev['group_id'] not in ab.keys():
                ab[rev['group_id']] = 0
            start_time=time.time()
            trigger_c=False
            b = ((trigger in rev['raw_message']  or (qq_name in rev['raw_message'] or '[CQ:at,qq=%d]' % rev['self_id'] in rev['raw_message'] or trigger_c == True or ab[rev['group_id']] > 0) and sstatus == True or random.randrange(0, 40) == 0) or (ab[rev['group_id']] > 0 and "[CQ:image," in rev['raw_message'])) and rev["sender"][
                "nickname"] not in ban_name
            ab[rev['group_id']] -= 1
            print("/"+str(ab[rev['group_id']]))
            ...
            if not os.path.exists("./user/g%s" % rev['group_id']):
                ...
                with open("./user/g%s/memory.txt" % rev['group_id'], "w") as tpass:
                    pass
            if "#1" in rev['raw_message']:
                print("即将发送小曲")
                res = requests.post(f'http://localhost:{socket_number}/send_group_msg', json={
                    'group_id': rev['group_id'],
                    'message': r"[CQ:record,file=C:\Users\86138\Desktop\qq聊天机器人\Qbot-main - 副本 - 副本\video_20250101_095639.mp3]"
                })
            bb = ""
            for i in rev['raw_message']:
                bb += i
            ...
            if "[CQ:image," in rev['raw_message'] and "AI" not in rev["sender"]["nickname"] and  rev["sender"]["nickname"] not in ban_name and "纳西妲" not in rev["sender"]["nickname"]and "春蓝" not in rev["sender"]["nickname"]\
                ...
                image = json.loads(response.text)["data"]["file"]
                with open(image, 'rb') as img_file:
                    img_base[rev['group_id']] = base64.b64encode(img_file.read()).decode('utf-8')
                print("运行到这里说明保存成功")
            ...
            if "#识图" in rev['raw_message']:
                data = {
                    ...
                    "messages": [
                        ...
                        "content": [
                                {"type": "text",
                                 "text": system + "请分析一下这张图片里的内容"},
                                {"type": "image_url",
                                    "image_url": {
                                        "url": f"data:image/png;base64,{img_base[rev['group_id']]}"
                                    }}
                            ]
                        }
                    ],
            ...
            if (b and len(rev['raw_message'].replace("[CQ:at,qq=","")) < max_ac_word and "*" not in rev['raw_message']) or (b and "[CQ:image," in rev['raw_message']):
                temp_msg_container[rev['group_id']] = ""
                start_time = time.time()
                status = False
                if '[CQ:at,qq=%d]' % rev['self_id'] in rev['raw_message'] or trigger in rev['raw_message'] or qq_name in rev['raw_message']:
                    ab[rev['group_id']] = 1
                if '[CQ:at,qq=%d]' % rev['self_id'] in rev['raw_message'] or trigger in rev['raw_message'] or qq_name in rev['raw_message'] or random.randrange(0, 10) == 0:
                    i = requests.post(f'http://localhost:{socket_number}/group_poke', json={
                        "group_id": rev['group_id'],
                        "user_id": rev['sender']['user_id']
                    })
                ...
                if '#暂停' in rev['raw_message'] and (
                        verification_code in rev['raw_message'] or rev['sender']["user_id"] == root_id):
                    send_msg({'msg_type': 'group', 'number': rev['group_id'], 'msg': '[进入待机状态]'})
                    print("已退出活跃状态")
                    pause_group = True
                    verification_code = str(random.randrange(100000, 1000000))
                if '#继续' in rev['raw_message'] and (
                ...
                if '#重置' in rev['raw_message'] and (
                    ...
                    if pause_group != True:
                        ...
                        memory = get_memory("./user/g%s/memory.txt" % rev['group_id'], keywords)
                        try:
                            with open ("./user/g%s/i_memory.txt" % rev['group_id'], "r", encoding="utf-8") as f:
                                i_memory = f.read()
                        except:
                            i_memory = ""
                            pass
                        print("重要记忆：" + i_memory)
                        print("-------------------------即将执行AI回复-----------------------------")
                        if "[CQ:image," not in rev['raw_message'] and img_base[rev['group_id']] == "":
                            data = {
                                "model": user_chat_model,
                                "messages": merge_contents([{"role": "system", "content": messages[0][
                                                                                              "content"] + "[memory](模糊 无时效性)\n%s\n" % memory + i_memory + "你现在是在参与群聊" +  e_information + temp_msg_container[rev['group_id']]}] + messages[
                                                                                                                                                                          1:]),
                                "stream": True,
                            }
                        elif img_base[rev['group_id']] != "":
                            bb = ""
                            for i in rev['raw_message']:
                                bb += i
                                if i == "[":
                            ...
                            data = {
                                "model": user_chat_model,
                                "messages":[{"role": "system", "content": "你是一个图像分析专家，你能结合你的所知准确完整的分析图片内容"},
                                            {"role": "user",
                                             "content": [
                                                        {"type": "text",
                                                         "text": "请尝试详细分析这张图片里的元素和内容"},
                            ...
                            data = {
                                "model": user_chat_model,
                                "messages": [{"role": "system",
                                              "content": system + "[memory](模糊 无时效性)\n%s\n" % memory + i_memory + "你现在是在参与群聊" + e_information},
                                             {"role": "user",
                                              "content": "%s给你发了一张图片，以下是关于这张图片的描述，请你对此做出回应。\n%s"%(rev["sender"]["nickname"],recognize_msg)}],
                                "stream": True,
                            }
                            img_base[rev['group_id']] = ""
                        is_return = True
                        while is_return:
                            ...
                            if len(temp_tts_list)==1:
                                ...
                                if '#summary/' in temp_tts_list[-2]:
                                    with open(
                                            "./user/g%s/i_memory.txt" % rev['group_id'],
                                            "a",
                                            encoding="utf-8")as f:
                                            f.write(temp_tts_list[-2].replace("#", "").replace("#summary/",""))
                                    temp_tts_list[-2] = ""
                                ...
                                if '#voice/' in temp_tts_list[-2]:
                                    ...
                                    send_voice(
                                        {'msg_type': 'group', 'number': rev['group_id'], 'msg': name})
                                    temp_tts_list[-2] = ""
                                if '#image/' in temp_tts_list[-2]:
                                    keyword = temp_tts_list[-2].split('#image/')[-1].replace("#", '')
                                    fliename = get_pixiv(keyword)
                                    send_msg({'msg_type': 'group', 'number': rev['group_id'],
                                              'msg': f"[CQ:image,file={absolute_path}\pixiv_images\{fliename}]"})
                                    temp_tts_list[-2] = ""
                                if '#bilibili/' in temp_tts_list[-2]:
                                    send_msg({'msg_type': 'group', 'number': rev['group_id'], 'msg': "稍等..."})
                                    search_data = search_bilibili(temp_tts_list[-2])
                                    if "&" in rev['raw_message']:
                                        a = ""
                                        for i in range(5):
                                            a += f"{i + 1}.{search_data[i]['title']}\n"
                                            a += f"{i + 1}.{search_data[i]['视频链接']}\n\n"
                                        send_msg({'msg_type': 'group', 'number': rev['group_id'], 'msg': a})
                                    if "&" not in rev['raw_message']:
                                        image_url = search_data[0]["封面图"]
                                        #save_path = save_image_from_url("https:" + image_url)
                                        send_msg({'msg_type': 'group', 'number': rev['group_id'], 'msg':f"[CQ:image,file=https:{image_url}]"})
                                        send_msg({'msg_type': 'group', 'number': rev['group_id'],
                                                  'msg':"视频标题:" + search_data[0]["title"] + "\n%s"%search_data[0]["视频链接"]})
                                        time.sleep(2)
                                    ...
                                    try:
                                        ...
                                        try:
                                            while lenn > 0:
                                                send_msg({'msg_type': 'group', 'number': rev['group_id'],
                                                          'msg': temp_tts_list[-lenn].replace("```", "").replace("[like]","").replace("#image/","")})
                                                lenn -= 1
                                            if random.randrange(0, 2) == 0:
                                                try:
                                                    name = voice(temp_tts_list[-2].replace("CQ", "").replace(rev['sender']['nickname'],"").replace(rev['sender']['user_id'],""))
                                                    send_voice(
                                                        {'msg_type': 'group', 'number': rev['group_id'], 'msg': name})
                                                except Exception as e:
                                                    print(e)
                ...
                temp_msg_container[rev['group_id']] = ("【" + "name=" +
                                                       rev["sender"]["nickname"] + "qq=" + str(
                            rev["sender"]["user_id"]) + "】说：" + rev['raw_message'].replace(
                            '[CQ:at,qq=%d]' % rev['self_id'], ''))
            print("未发现新消息...运行时间：%f" % (time.time() - startT))
...
def main2():
    ...
    while active==True:
        ...
        time.sleep(5)
        # if "illue%schat" % active_private not in objdict.keys():
        #     objdict["illue%schat" % active_private][0] = ""
        try:
            with open("./user/p%s/i_memory.txt" %active_private, "r", encoding="utf-8") as f:
                i_memory = f.read()
        except:
            i_memory = ""
            pass
        if goal_time >= 8:
            sstatus = True
        elif time.strftime("%H", time.localtime()) == set_time and goal_time >= random.randrange(1200, 1800):
            print("即将触发主动分享")
            ...
            content,think_con = request_api(data)
            send_msg({'msg_type': 'group', 'number': active_group,
                          'msg': content})
        elif active_goal == True:
            if time.strftime("%H", time.localtime()) == "07" and a==False:
                print("重要记忆：" + i_memory)
                keywords = jieba.analyse.extract_tags("早上", topK=5)
                memory = get_memory("./user/p%s/memory.txt" % active_private, keywords)
                data = {
                    "model": user_chat_model,
                    "stream": True,
                    "messages": [
                        {"role": "system",
                         "content": system},
                        {"role": "user",
                         "content": memory + i_memory+"以上是你的记忆\n%s现在是早上了，是在私聊场景下，请你和用户打个招呼吧(系统提示音)"%time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        }],
                    "max_tokens": 400
                }
                content, think_con = request_api(data)
                send_msg({'msg_type': 'private', 'number': active_private,
                          'msg': content.replace("#","").replace("cut","")})
                # objdict["illue%schat" % active_private][0] = objdict["illue%schat" % active_private][0] + [
                #     {'role': 'user', 'content': "以上是你的记忆\n%s现在是早上了，是在私聊场景下，请你和用户打个招呼吧(系统提示音)"%time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())},
                #     {'role': 'assistant', 'content': think_con}]
                a = True
            if time.strftime("%H", time.localtime()) == "00" and a  == True:
                print("重要记忆：" + i_memory)
                keywords = jieba.analyse.extract_tags("晚上", topK=5)
                memory = get_memory("./user/p%s/memory.txt" % active_private, keywords)
                data = {
                    "model": user_chat_model,
                    "stream": True,
                    "messages": [
                        {"role": "system",
                         "content": system},
                        {"role": "user",
                         "content":  memory + i_memory+"以上是你的记忆\n%s现在是晚上了，是在私聊场景下，请你和用户打个招呼吧(系统提示音)" % time.strftime(
